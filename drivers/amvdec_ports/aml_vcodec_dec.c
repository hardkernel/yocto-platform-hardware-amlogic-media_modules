...
void __aml_vdec_dispatch_event(struct aml_vcodec_ctx *ctx, u32 changes, struct set_param_info *param)
{
	...
	if ((changes != V4L2_EVENT_SRC_CH_HDRINFO) &&
		(changes != V4L2_EVENT_REPORT_DEC_INFO) &&
		(changes != V4L2_EVENT_REPORT_SIGNAL_TYPE))
...
static bool is_there_enough_yuv_dmabuf(struct aml_vcodec_ctx *ctx, struct dma_buf *dbuf)
{
	...
	v4l_dbg(ctx, V4L_DEBUG_CODEC_BUFMGR,
				...
				ctx->picinfo.dpb_margin,
				ctx->dpb_size,
				ctx->bm.bc.dma_free_num,
				ctx->alloced_yuv_num);
	if (ctx->alloced_yuv_num < (ctx->picinfo.dpb_frames + ctx->picinfo.dpb_margin))
		return true;
				  ...
				  struct vb2_v4l2_buffer *vb)
{
	...
	v4l_dbg(ctx, V4L_DEBUG_CODEC_BUFMGR,
		...
		(ulong)am_buf->dma->phy_addr, (u32)mbuf->size, mbuf->ion_flags,
...
void aml_v4l_ctx_release(struct kref *kref)
{
	...
	aml_buf_deinit_dma(&ctx->bm);
	aml_es_mgr_release(ctx);

	aml_media_mem_free(ctx);
}

static bool aml_ubuf_queue_check(struct vb2_buffer *vb)
{
	...
	if (!list_empty(&ctx->ubuf_que)) {
		list_for_each_entry(ubuf, &ctx->ubuf_que, node) {
			if (dbuf == ubuf->dbuf) {
				ubuf->index	= vb->index;
				ubuf->addr	= get_addr(vb, 0);
				am_buf->uvm_buf = (void *)ubuf;
				v4l_dbg(ctx, V4L_DEBUG_CODEC_BUFMGR,
					"%s, update, ubuf:%px vb:%d, dbuf:%px, addr:%lx ino:%lu\n",
					__func__, ubuf, ubuf->index, ubuf->dbuf, ubuf->addr,
...
static void aml_ubuf_queue_add(struct aml_vcodec_ctx *ctx, struct aml_uvm_buff_ref *ubuf)
{
	if (!ctx->bm.config.dynamic_mode)
		return;

	mutex_lock(&ctx->ubuf_lock);
	list_add_tail(&ubuf->node, &ctx->ubuf_que);
	mutex_unlock(&ctx->ubuf_lock);
}
		...
		if (!aml_buf_check_in_table(&ctx->bm, (ulong)vb->planes[0].dbuf) && aml_buf_is_dynamic_mode_inited(&ctx->bm)) {
			...
			if (!ret && (buf->aml_buf->pair_state == MASTER_DONE ||
				...
				for (; buf->aml_buf->pair_state < PAIR_DONE && !ret;) {
					...
					if (!buf_pair || ret)
						break;
					vb2_v4l2 = container_of(buf_pair->vb, struct vb2_v4l2_buffer, vb2_buf);
					ret = aml_uvm_buf_delay_alloc(ctx, vb2_v4l2);
					if (ret)
						break;
					if (buf_pair->master_buf)
						buf_pair = (struct aml_buf *)buf_pair->master_buf;
...
static const struct v4l2_ctrl_config ctrl_st_screenmode = {
	.name	= "screenmode",
	.id	= AML_V4L2_SET_SCREEN_MODE,
	.ops	= &aml_vcodec_dec_ctrl_ops,
	.type	= V4L2_CTRL_TYPE_INTEGER,
	.flags	= V4L2_CTRL_FLAG_WRITE_ONLY,
	.min	= 0,
