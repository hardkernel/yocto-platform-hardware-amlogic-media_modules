...
/**
 * struct h266_fb - h266 decode frame buffer information
...
/**
 * struct vdec_h266_dec_info - decode information
...
/**
 * struct vdec_h266_vsi - shared memory for decode information exchange
 ...
 */
struct vdec_h266_vsi {
	char *header_buf;
	int sps_size;
	int pps_size;
	int sei_size;
	int head_offset;
	struct vdec_h266_dec_info dec;
...
/**
 * struct vdec_h266_inst - h266 decoder instance
 * @num_nalu : how many nalus be decoded
 * @ctx      : point to aml_vcodec_ctx
 * @vsi      : VPU shared information
 */
struct vdec_h266_inst {
	unsigned int num_nalu;
	struct aml_vcodec_ctx *ctx;
	struct aml_vdec_adapt vdec;
	struct vdec_h266_vsi *vsi;
...
static int vdec_write_nalu(struct vdec_h266_inst *inst,
	u8 *buf, u32 size, u64 ts, chunk_free free);

static void get_pic_info(struct vdec_h266_inst *inst,
			 struct vdec_pic_info *pic)
{
	*pic = inst->vsi->pic;
...
static void get_crop_info(struct vdec_h266_inst *inst, struct v4l2_rect *cr)
{
...
static void get_dpb_size(struct vdec_h266_inst *inst, unsigned int *dpb_sz)
{
...
static u32 vdec_config_default_parms(u8 *parm)
{
	...
	pbuf += sprintf(pbuf, "h266_double_write_mode:3;");
	pbuf += sprintf(pbuf, "h266_buf_width:1920;");
	pbuf += sprintf(pbuf, "h266_buf_height:1088;");
	pbuf += sprintf(pbuf, "h266_max_pic_w:8192;");
...
static void vdec_parser_parms(struct vdec_h266_inst *inst)
{
	...
	if (ctx->config.parm.dec.parms_status &
		...
		pbuf += sprintf(pbuf, "h266_double_write_mode:%d;",
			ctx->config.parm.dec.cfg.double_write_mode);
		pbuf += sprintf(pbuf, "h266_triple_write_mode:%d;",
...
static int vdec_h266_init(struct aml_vcodec_ctx *ctx, unsigned long *h_vdec)
{
	struct vdec_h266_inst *inst = NULL;
	int ret = -1;
	...
	inst->vdec.frm_name	= "H266";
	inst->vdec.video_type	= VFORMAT_H266;
	...
	inst->vsi = aml_media_mem_alloc(sizeof(struct vdec_h266_vsi), GFP_KERNEL);
	if (!inst->vsi) {
		ret = -ENOMEM;
		goto err;
	}
...
static int parse_stream_ucode(struct vdec_h266_inst *inst,
...
static int parse_stream_ucode_dma(struct vdec_h266_inst *inst,
...
static int parse_stream_cpu(struct vdec_h266_inst *inst, u8 *buf, u32 size)
{
	v4l_dbg(inst->ctx, V4L_DEBUG_CODEC_ERROR,
...
static void vdec_h266_deinit(unsigned long h_vdec)
{
	struct vdec_h266_inst *inst = (struct vdec_h266_inst *)h_vdec;
	struct aml_vcodec_ctx *ctx = inst->ctx;
...
static bool monitor_res_change(struct vdec_h266_inst *inst, u8 *buf, u32 size)
{
	int ret = -1;
	u8 *p = buf;
	int len = size;
...
static int vdec_h266_decode(unsigned long h_vdec,
			   struct aml_vcodec_mem *bs, bool *res_chg)
{
 ...
 static void get_param_config_info(struct vdec_h266_inst *inst,
	struct aml_dec_params *parms)
 {
...
static void get_param_comp_buf_info(struct vdec_h266_inst *inst,
		struct vdec_comp_buf_info *params)
{
	memcpy(params, &inst->comp_info, sizeof(*params));
}

static int vdec_h266_get_param(unsigned long h_vdec,
			       enum vdec_get_param_type type, void *out)
{
	...
	if (!inst) {
		v4l_dbg(inst->ctx, V4L_DEBUG_CODEC_ERROR,
			"the h266 inst of dec is invalid.\n");
		return -1;
	}
...
static void set_param_write_sync(struct vdec_h266_inst *inst)
{
	complete(&inst->comp);
}

static void set_param_ps_info(struct vdec_h266_inst *inst,
	struct aml_vdec_ps_infos *ps)
{
	struct vdec_pic_info *pic = &inst->vsi->pic;
	struct vdec_h266_dec_info *dec = &inst->vsi->dec;
...
static void set_param_comp_buf_info(struct vdec_h266_inst *inst,
		struct vdec_comp_buf_info *info)
{
	memcpy(&inst->comp_info, info, sizeof(*info));
}

static void set_param_hdr_info(struct vdec_h266_inst *inst,
...
static void set_param_post_event(struct vdec_h266_inst *inst, u32 *event, struct set_param_info *param)
{
	aml_vdec_dispatch_event(inst->ctx, *event);
	v4l_dbg(inst->ctx, V4L_DEBUG_CODEC_PROT,
		"h266 post event: %d, fun: %s, %d\n",
		param->event, param->function, param->line);
}

static void set_pic_info(struct vdec_h266_inst *inst,
	struct vdec_pic_info *pic)
{
	inst->vsi->pic = *pic;
}

static void set_cfg_info(struct vdec_h266_inst *inst,
...
static int vdec_h266_set_param(unsigned long h_vdec,
...
static struct vdec_common_if vdec_h266_if = {
	.init		= vdec_h266_init,
	.probe		= vdec_h266_probe,
	.decode		= vdec_h266_decode,
	.get_param	= vdec_h266_get_param,
	.set_param	= vdec_h266_set_param,
	.deinit		= vdec_h266_deinit,
};

struct vdec_common_if *get_h266_dec_comm_if(void)
{
	return &vdec_h266_if;
}
