...
#define MULTI_DRIVER_NAME "ammvdec_h266"
#endif
#define DRIVER_NAME "amvdec_h266"
...
static void release_aux_data(struct hevc_state_s *hevc,
	struct PIC_s *pic);
void release_pic_mmu_buf(struct hevc_state_s *hevc, struct PIC_s *pic);
...
static int init_mmu_buffers(struct hevc_state_s *hevc, int bmmu_flag)
{
	...
	if (hevc->mmu_enable) {
		hevc->mmu_box = decoder_mmu_box_alloc_box(DRIVER_NAME,
			...
			tvp_flag);
		if (!hevc->mmu_box) {
			pr_err("h266 alloc mmu box failed!!\n");
			return -1;
		}
#ifdef VVC_10B_MMU_DW
...
static int config_pic(struct hevc_state_s *hevc, struct PIC_s *pic)
{
	...
	if (get_dbg_flag(hevc) & H266_DEBUG_BUFMGR) {
		hevc_print(hevc, 0,
		"%s index %d BUF_index %d bufadr %lx\n",
		 __func__, pic->index, pic->BUF_index, hevc->m_BUF[i].start_adr);

		if (hevc->mmu_enable && dw_mode)
			hevc_print(hevc, 0,
			"%s, dw_nv21 y_adr %x, uv_adr %x\n",
...
static void config_sao_hw(struct hevc_state_s *hevc)
{
	...
	if (is_dw_p010(hevc))
		mc_buffer_size_u_v_h <<= 1; //64k alignment and double for P010
	...
	if (dw_mode && ((dw_mode & 0x20) == 0)) {
		...
		WRITE_VREG(HEVC_SAO_Y_LENGTH, (data32 << 1));
	...
	data32 = (mc_buffer_size_u_v_h << 16);
	WRITE_VREG(HEVC_SAO_Y_LENGTH3, (data32 << 1));
	...
	WRITE_VREG(HEVC_SAO_C_START_ADDR3, pic->mc_u_v_adr;);
#endif
#endif
	/*
	copy from h265:
...
static void print_pic_pool(struct hevc_state_s *hevc, char *mark)
{
	...
	hevc_print(hevc, 0, "%s----pic_pool (used %d, total %d):\n", mark, used_count, PIC_POOL_SIZE);
	for (i = 0; i < PIC_POOL_SIZE; i++) {
		pic = &hevc->vvc_dec->pic_pool[i];
		if (pic->used) {
			hevc_print(hevc, 0, "%d(%d,%d): decode_idx %d, mv_idx %d, %spoc %d, referenced %d, decode_done %d, error_mark 0x%x, vf_ref %d, w/h/depth(%d,%d,%d), slicetype %d, mv_wr_start 0x%x\n",
			i, pic->index, pic->BUF_index, pic->decode_idx,
#ifndef MV_USE_FIXED_BUF
			pic->mv_buf_index,
#else
			-1,
...
void release_pic_mmu_buf(struct hevc_state_s *hevc,
	struct PIC_s *pic)
{
	hevc_print(hevc, H266_DEBUG_BUFMGR_MORE,
...
static void vh266_vf_put(struct vframe_s *vf, void *op_arg)
{
	...
	if (index_top != 0xff
		...
		if (hevc->vvc_dec->pic_pool[index_top].vf_ref > 0) {
			hevc->vvc_dec->pic_pool[index_top].vf_ref--;

			if (hevc->vvc_dec->pic_pool[index_top].vf_ref == 0) {
				hevc->vvc_dec->pic_pool[index_top].show_frame = false;
	...
	if (index_bot != 0xff
		...
		if (hevc->vvc_dec->pic_pool[index_bot].vf_ref > 0) {
			hevc->vvc_dec->pic_pool[index_bot].vf_ref--;

			if (hevc->vvc_dec->pic_pool[index_bot].vf_ref == 0) {
				hevc->vvc_dec->pic_pool[index_bot].show_frame = false;
...
muti_output:
	...
	if (dec_status == HEVC_SEI_DAT) {
		...
		if (vvc_dec->img.width != param->p.pic_width_in_luma_samples ||
		vvc_dec->img.height != param->p.pic_height_in_luma_samples) {
			//vvc_dec->init_hw_flag = 0; //???
			hevc_print(hevc, H266_DEBUG_BUFMGR, "width/height (%d/%d) change to %d/%d, set init_hw_flag to 0\n",
				vvc_dec->img.width, vvc_dec->img.height,
				param->p.pic_width_in_luma_samples, param->p.pic_height_in_luma_samples
				);
		}
		...
		if (ret == 0) {
			if (p_declib->m_pcPic) {
				...
				vvc_dec->cur_pic->width = param->p.pic_width_in_luma_samples;
				vvc_dec->cur_pic->height = param->p.pic_height_in_luma_samples;
				vvc_dec->cur_pic->slice_type = param->p.slice_type;
				vvc_dec->cur_pic->depth = param->p.sps_bitdepth_minus8 + 8;
				vvc_dec->cur_pic->poc = p_declib->m_pcPic->poc;
				//vvc_dec->cur_pic->longTerm = p_declib->m_pcPic->longTerm;
				vvc_dec->vvc_monochrome = (param->p.sps_chroma_format_idc == 0);
				vvc_dec->lcu_size_log2 = param->p.lcu_size;
				vvc_dec->lcu_size = 1 << vvc_dec->lcu_size_log2;
				vvc_dec->lcu_x_num  = (vvc_dec->cur_pic->width % vvc_dec->lcu_size) ?
				vvc_dec->cur_pic->width / vvc_dec->lcu_size + 1 : vvc_dec->cur_pic->width / vvc_dec->lcu_size;
				vvc_dec->lcu_y_num = (vvc_dec->cur_pic->height % vvc_dec->lcu_size ) ?
				vvc_dec->cur_pic->height/vvc_dec->lcu_size + 1 : vvc_dec->cur_pic->height/vvc_dec->lcu_size;
				vvc_dec->lcu_total = vvc_dec->lcu_x_num * vvc_dec->lcu_y_num;
				vvc_dec->slice_type = vvc_dec->cur_pic->slice_type;
				vvc_dec->slice_addr = param->p.sliceAddr;
				vvc_dec->cur_pic->inter_slice_allowed_flag = (param->p.slice_ph_decoding_flags_0 >> 12) & 0x1;
				if (vvc_dec->slice_type != I_SLICE)
				...
				if (get_dbg_flag(hevc) & H266_DEBUG_BUFMGR_MORE)
					print_pic_pool(hevc, "after bufmgr_process");
				hevc->new_pic = vvc_dec->cur_pic->new_picture;
				if (vvc_dec->cur_pic->new_picture) {
					vvc_frame_t *new_pic = vvc_dec->cur_pic;
...
#ifdef VVC_10B_MMU_DW
					if (hevc->dw_mmu_enable) {
						ret = H266_alloc_mmu_dw(hevc, new_pic,
							param->p.sps_bitdepth_minus8,
							hevc->frame_dw_mmu_map_addr);
						if (ret != 0) {
							put_mv_buf(new_pic);
	...
	void (*callback)(struct vdec_s *, void *, int), void *arg)
{
	...
	if (vdec->mc_loaded) {
		...
		*/
		...
		//pr_info("%s, load h266 ucode. swap %d\n", __func__, hevc->enable_ucode_swap);
		if (hevc->enable_ucode_swap) {
...
static KV_INT_TO_VOID amvdec_h266_remove(struct platform_device *pdev)
{
	struct hevc_state_s *hevc = gHevc;
...
static KV_INT_TO_VOID ammvdec_h266_remove(struct platform_device *pdev)
{
	struct hevc_state_s *hevc =
		(struct hevc_state_s *)
...
static int __init amvdec_h266_driver_init_module(void)
{
	...
	if (debug & PRINT_FLAG_VDEC_DETAIL)
		pr_info("amvdec_h266 module init\n");
	...
	vcodec_profile_register_v2("h266", VFORMAT_H266, 0);
	INIT_REG_NODE_CONFIGS("media.decoder", &decoder_266_node,
		"h266", h266_configs, CONFIG_FOR_RW);
	vcodec_feature_register(VFORMAT_H266, 0);
...
{
	pr_debug("amvdec_h266 module remove.\n");
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
static struct param_entry amvdec_h266_params[] = {
	PARAM_UINT(use_cma),
	...
	PARAM_UINT(frmbase_muti_slice),
	{ /* sentinel */ }
};
module_param_cb(params, &key_value_param_ops, &amvdec_h266_params, 0644);
...
#ifndef MULTI_INSTANCE_SUPPORT
MEDIA_PARAM(workaround_enable, uint, 0664);
...
MEDIA_PARAM_ARRAY(max_get_frame_interval,
...
MEDIA_PARAM_ARRAY(ref_frame_mark_flag, uint,
	&max_decode_instance_num, 0664);
...
MEDIA_PARAM(force_dpb_size, uint, 0664);
...
MEDIA_PARAM(frmbase_muti_slice, uint, 0664);
