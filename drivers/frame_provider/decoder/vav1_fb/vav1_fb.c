...
static void timeout_process(struct AV1HW_s *hw)
{
	...
	av1_print(hw, 0, "%s decoder timeout, lcu 0x%x HEVC_MPC_E=0x%x\n",
		__func__, READ_VREG(HEVC_PARSER_LCU_START), READ_VREG(HEVC_MPC_E));
#ifdef NEW_FB_CODE
		if (hw->front_back_mode == 1) {
			amhevc_stop_f();
		} else
...
/****************************************/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
static struct param_entry amvdec_av1_fb_params[] = {
#ifdef CONFIG_AMLOGIC_MEDIA_ENHANCEMENT_DOLBYVISION
...
module_param_cb(params, &key_value_param_ops, &amvdec_av1_fb_params, 0644);
#endif
...
MEDIA_PARAM(disable_force_dw, uint, 0664);
