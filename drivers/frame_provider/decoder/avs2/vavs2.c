...
#define DYN_CACHE  //!!!enable it on s5 t3x s6
...
static void dump_pic_list(struct AVS2Decoder_s *dec)
{
	...
	for (ii = 0; ii < avs2_dec->ref_maxbuffer; ii++) {
		avs2_print(dec, 0,
		"fref[%d]: index %d decode_id %lld mvbuf %d imgcoi_ref %d imgtr_fwRefDistance %d referred %d, pre %d is_out %d, bg %d, vf_ref %d error %d lcu %d ref_pos(%d,%d,%d,%d,%d,%d,%d), time %lld\n",
		ii, avs2_dec->fref[ii]->index,
...
static int avs2_local_init(struct AVS2Decoder_s *dec)
{
	...
	if (cuva_buf_size > 0) {
		...
		dec->cuva_addr = decoder_dma_alloc_coherent(&dec->cuva_handle,
	        ...
	        avs2_print(dec, AVS2_DBG_BUFMGR,
			"%s, cuva_size = %d cuva_phy_addr %lx dec->cuva_addr = %px\n",
...
static int ammvdec_avs2_probe(struct platform_device *pdev)
{
	...
	if (!is_support_triple_write() && get_triple_write_mode(dec)) {
		triple_write_mode = 0;
		dec->triple_write_mode = 0;
		pr_err("%s warn: unsupport tw, disabled tw\n", __func__);
	}
#endif
#ifdef P010_ENABLE
	if (!is_support_p010_mode()) {
