	...
	struct PIC_BUFFER_CONFIG_s *pic)
{
	...
	if (debug) {
		...
		pr_info("comp_body_size %x comp_buf_size %x ",
			pic->comp_body_size,
			pic->buf_size);
		pr_info("mpred_mv_wr_start_adr %lx\n",
			pic->mpred_mv_wr_start_addr);
	...
	if ((pbi->work_space_buf->mpred_mv.buf_start + ((i + 1) * mv_size)) <= mpred_mv_end) {
		...
		}
		{
			...
			if (debug) {
				...
				pr_info("tw y_addr %lx, uv_addr %lx\n",
	...
	if (get_double_write_mode(pbi) & 0x10) {
		if (is_dw_p010(pbi)) {
			...
			WRITE_VREG(HEVCD_MPP_DECOMP_CTL1,
				(0x1 << 31) | (8 << 24) | (((pbi->endian >> 12) & 0xff) << 16));
		} else {
...
static int ammvdec_vp9_probe(struct platform_device *pdev)
{
	...
	if (triple_write_mode)
		pbi->triple_write_mode = get_triple_write_mode(pbi);

	if (!is_support_triple_write() && get_triple_write_mode(pbi)) {
		triple_write_mode = 0;
...
/****************************************/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
static struct param_entry amvdec_vp9_params[] = {
	...
	PARAM_UINT(vp9_max_pic_w),
	PARAM_UINT(vp9_max_pic_h),
...
module_param_cb(params, &key_value_param_ops, &amvdec_vp9_params, 0644);
#endif
...
MEDIA_PARAM(vp9_max_pic_w, uint, 0664);
MODULE_PARM_DESC(vp9_max_pic_w, "\n vp9_max_pic_w\n");

MEDIA_PARAM(vp9_max_pic_h, uint, 0664);
