...
void av1_raw_write_image(AV1Decoder *pbi, PIC_BUFFER_CONFIG *sd)
{
	...
	if (hw->enable_fence && (sd->fence_create == 1)) {
		...
		if (hw->t5d_fg_run_rotate != T5D_ROTATE_DW3) {
			av1_print(hw, AOM_DEBUG_HW_MORE, "Out frame index %d not_need_display %d, error_mark %d, order_hint %d, idx %d\n",
...
MEDIA_PARAM(fgs_valid, uint, 0664);
