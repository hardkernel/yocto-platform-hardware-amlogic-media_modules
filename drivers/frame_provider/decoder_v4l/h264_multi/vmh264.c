...
static void h264_post_avbcd_task(struct vdec_h264_hw_s *hw)
{
	...
	dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_STATUS,
			"%s: block mode 0x%x bit_depth_luma %d (vf %px header_addr 0x%lx y_addr 0x%lx wxh %d x %d bitdepth %d type 0x%lx index %d"
			" offset %d)\n",
			__func__, hw->canvas_mode, bitdepth, vf, (ulong)in->img.data, am_buf->planes[0].addr, in->img.rect.width, in->img.rect.height, in->img.bitdep,
...
//di_post not support PIC_DOUBLE_FRAME and PIC_TRIPLE_FRAME
	...
	for (i = 0; i < vf_count; i++) {
		...
		if (vdec_stream_based(vdec)) {
			...
			if (i == 0) {
				dur_offset = ((dur_offset << 32 | (frame_type << PTS_US64_FRAME_TYPE_SHIFT)) & 0xffffffff00000000) | offset;
		...
		dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_STATUS,
			"%s: vf %px index %d poc %d frame_type %d dur %d type %x pts %d(0x%x), pts64 %lld(0x%x) ts %llu(0x%x)\n",
...
static int check_ip_change(struct vdec_h264_hw_s *hw, u32 param1, u32 param2)
{
	int mb_width, mb_total;
	int mb_height = 0;
	...
	field = (!hw->is_interlace) && frame_mbs_only_flag ?
		V4L2_FIELD_NONE : V4L2_FIELD_INTERLACED;
	field = check_force_interlace(hw, frame_width, frame_height) ?
		V4L2_FIELD_INTERLACED : field;

	return (hw->field != field);
}
	...
	u32 param1, u32 param2, u32 param3, u32 param4, bool buffer_reset_flag, bool reset_flags)
{
	...
	if ((((seq_info2 != 0 &&
		(hw->last_res_info.param1 & 0xffffff) != (param1 & 0xffffff)) || dec_dpb_size_change ||
		check_ip_change(hw, param1, param2)) && hw->last_res_info.param1 != 0
		...
		if (!reset_flags) {
			...
			dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_STATUS,
				...
				used_reorder_dpb_size_margin);
			/*hw->bForceInterlace is only updated when reporting a resolution switch event.*/
			if (hw->bForceInterlace &&
				(hw->is_interlace != hw->last_res_info.is_interlace) &&
				(hw->last_res_info.max_ref_size !=
				((param1 >> 24) & 0xf))) {
				flush_dpb(p_H264_Dpb);
				bufmgr_h264_remove_unused_frame(p_H264_Dpb, 0);
				update_ref_list(&p_H264_Dpb->mDPB);
			}
		}
	...
	struct h264_dpb_stru *p_H264_Dpb, u8 *sei_data_buf, u32 data_len, struct userdata_meta_info_t *meta_info)
{
	...
	dpb_print(DECODE_ID(hw), PRINT_FLAG_SEI_DETAIL,	"%s, ud ready event report\n", __func__);
}
...
int set_mmu_config(struct vdec_h264_hw_s *hw, struct vdec_s *vdec)
{
...

	{
		...
		struct firmware_s *fw_mmu = fw_firmware_s_creat(fw_mmu_size);
		int size;
		if (!fw_mmu)
			return -ENOMEM;
	...
	u32 param1, u32 param2, u32 param3, u32 param4)
{
	...
	if (ctx->param_sets_from_ucode &&
		...
		if (((param1 != 0 &&
			(hw->last_res_info.param1 & 0xffffff) != (param1 & 0xffffff)) || dec_dpb_size_change ||
			check_ip_change(hw, param1, param2)) && hw->last_res_info.param1 != 0) { /*picture size changed*/
			...
			if (!v4l_res_change(hw, param1, param2, param3, param4)) {
				if (!hw->v4l_params_parsed) {
					...
					if (v4l_resolution_double_check(hw, param1, param2, param3, param4)) {
						...
						hw->last_res_info.max_ref_size = (param1 >> 24) & 0xf;
						hw->last_res_info.is_interlace = hw->is_interlace;
					}
					amvdec_stop();
					if (hw->mmu_enable && !is_vdec_hevc_combine()) {
						amhevc_stop();
...
static int ammvdec_h264_probe(struct platform_device *pdev)
{
	...
	if (decoder_bmmu_box_alloc_idx_wait(hw->bmmu_box, BMMU_DPB_IDX,
		...
		align_2n, -1, BMMU_ALLOC_FLAGS_WAIT) < 0) {
		h264_free_hw_stru(&pdev->dev, (void *)hw);
...
/****************************************/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
static struct param_entry amvdec_h264_v4l_params[] = {
	...
	PARAM_UINT(fence_ignore_gap_error),
	PARAM_UINT(adjust_dpb_size),
...
MEDIA_PARAM(v4l_error_policy, uint, 0664);
...
MEDIA_PARAM(fence_ignore_gap_error, uint, 0664);
MODULE_PARM_DESC(fence_ignore_gap_error, "\n fence ignore gap error\n");

MEDIA_PARAM(adjust_dpb_size, uint, 0664);
