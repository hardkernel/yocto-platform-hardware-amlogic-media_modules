	...
	unsigned int *mmu_index_adr)
{
	...
	dec->avs3_dec.cur_pic->cur_mmu_4k_number = aml_buf->fbc->frame_size;
...
static void avs3_report_err_timestamp_for_decoded_pic(struct aml_vcodec_ctx *ctx,
	struct avs3_frame_s *pic)
{
	u64 timestamp_bak;
...
int find_near_pic_index(struct AVS3Decoder_s *dec)
{
	...
	for (i = 0; i < BUF_FBC_NUM_MAX; i++) {
		...
		if (abs(cur_pic->poc - afbc_buf->POC) < diff) {
			diff = abs(cur_pic->poc - afbc_buf->POC);
			index = i;
		}
	}
		...
		if (cur_pic->decoded_lcu != dec->avs3_dec.lcu_total) {
			...
			if (afbc_buf != NULL) {
				...
				avs3_print(dec, PRINT_FLAG_VDEC_DETAIL,
					...
					cur_pic->poc, afbc_buf->POC, index);
			}
		}
	}
...
static void avs3_recycle_mmu_buf_tail(struct AVS3Decoder_s *dec)
{
	...
	if (dec->cur_fb_idx_mmu != INVALID_IDX) {
		...
		avs3_print(dec, AVS3_DBG_BUFMGR_MORE,
			...
			__func__, aml_buf->fbc->index, used_4k_num, used_4k_num1);
		...
		if (dec->dw_mmu_enable) {
			...
			decoder_mmu_box_free_idx_tail(aml_buf->fbc->mmu_dw, aml_buf->fbc->index, used_4k_num);
#ifdef NEW_FB_CODE
			if ((dec->front_back_mode == 1) || (dec->front_back_mode == 3))
				decoder_mmu_box_free_idx_tail(aml_buf->fbc->mmu_dw_1, aml_buf->fbc->index, used_4k_num1);
#endif
		}
#endif
		if (pic->need_mmu_copy == 0)
...
#ifdef AVS3_10B_MMU_DW
		if (dec->dw_mmu_enable) {
			decoder_mmu_box_free_idx(aml_buf->fbc->mmu_dw,
				aml_buf->fbc->index);
#ifdef NEW_FB_CODE
			if (dec->front_back_mode)
				decoder_mmu_box_free_idx(aml_buf->fbc->mmu_dw_1,
					aml_buf->fbc->index);
#endif
			avs3_print(dec, AVS3_DBG_BUFMGR_MORE,
...
static void avs3_buf_ref_process_for_exception(struct AVS3Decoder_s *dec, bool is_front)
{
	...
	struct aml_vcodec_ctx *ctx =
		(struct aml_vcodec_ctx *)(dec->v4l2_ctx);
	struct aml_buf *aml_buf = NULL;
	int cur_idx = is_front ? dec->cur_idx: dec->cur_back_idx;
...
static irqreturn_t vavs3_isr_thread_fn(int irq, void *data)
{
	...
	if (dec_status == AVS3_DECODE_BUFEMPTY) {
		...
		if (pic != NULL) {
			...
			if (dec->mmu_enable) {
				struct aml_buf *aml_buf = index_to_aml_buf(dec, pic->BUF_index);
...
static KV_INT_TO_VOID amvdec_avs3_remove(struct platform_device *pdev)
{
	struct AVS3Decoder_s *dec = &gAVS3Decoder;
	if (debug)
	...
	if ((dec->front_back_mode == 0)
		...
		if (index != INVALID_IDX) {
			...
			if ((mmu_copy->mmu_copy_buf_start) &&
				(dec->avs3_dec.cur_pic->need_mmu_copy != 2) &&
...
void get_mmu_copy_map(struct AVS3Decoder_s *dec, struct avs3_frame_s *pic, u32 page_num)
{
	...
	if (buffer_size > mmu_copy->mmu_copy_buf_size) {
		...
		avs3_print(dec, PRINT_FLAG_VDEC_STATUS,
			"%s, index %d mmu_copy_buf_start 0x%x, buffer_size: %d, cost time = %d\n",
			__func__, aml_buf->fbc->index, mmu_copy->mmu_copy_buf_start,
...
static int ammvdec_avs3_probe(struct platform_device *pdev)
{
	...
	avs3_print(dec, 0, "double_write_mode 0x%x, Correction: policy 0x%x mode %d lcu_percentage_threshold %d\n",
...
static KV_INT_TO_VOID ammvdec_avs3_remove(struct platform_device *pdev)
{
...
/****************************************/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
static struct param_entry amvdec_avs3_v4l_params[] = {
	...
	PARAM_UINT(dump_phy_adr),
	PARAM_UINT(dump_phy_size),
	...
	PARAM_UINT(fast_timer_check_count),
	PARAM_UINT_ARRAY(max_process_time_back),
	PARAM_UINT(efficiency_mode),
#endif
	PARAM_UINT(trace_debug_mode),
	{ /* sentinel */ }
};
module_param_cb(params, &key_value_param_ops, &amvdec_avs3_v4l_params, 0644);
#endif
...
MEDIA_PARAM(used_dpb_size, uint, 0664);
...
MEDIA_PARAM(dump_phy_adr, uint, 0664);
MODULE_PARM_DESC(dump_phy_adr, "\n dump_phy_adr\n");

MEDIA_PARAM(dump_phy_size, uint, 0664);
...
MEDIA_PARAM(fast_timer_check_count, uint, 0664);
...
MEDIA_PARAM(trace_debug_mode, uint, 0664);
