	...
	while (kfifo_peek(&hevc->display_q, &vf) && vf) {
		...
		if (((hevc->front_back_mode) && (pic->back_done_mark)) ||
			...
			if (vf->type & VIDTYPE_V4L_EOS) {
				hevc_print(hevc, PRINT_FLAG_VDEC_DETAIL, "[%d] H265 EOS notify.\n", ctx->id);
				break;
			}
#ifdef NEW_FB_CODE
		} else {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
static struct param_entry amvdec_h265_fb_v4l_params[] = {
	PARAM_UINT(use_cma),
	...
	PARAM_UINT(pre_decode_buf_level),
...
module_param_cb(params, &key_value_param_ops, &amvdec_h265_fb_v4l_params, 0644);
#endif
