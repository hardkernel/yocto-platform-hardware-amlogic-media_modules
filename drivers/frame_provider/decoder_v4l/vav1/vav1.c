...
static void av1_post_avbcd_task(struct AV1HW_s *hw)
{
	...
	av1_print(hw, AV1_DEBUG_BUFMGR,
			...
			" offset %d)\n",
			__func__, hw->mem_map_mode, vf, (ulong)in->img.data, am_buf->planes[0].addr, in->img.rect.width, in->img.rect.height, in->img.bitdep,
...
/****************************************/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
static struct param_entry amvdec_av1_v4l_params[] = {
#ifdef CONFIG_AMLOGIC_MEDIA_ENHANCEMENT_DOLBYVISION
...
module_param_cb(params, &key_value_param_ops, &amvdec_av1_v4l_params, 0644);
#endif
