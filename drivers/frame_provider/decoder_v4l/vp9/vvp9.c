...
static void set_frame_info(struct VP9Decoder_s *pbi, struct vframe_s *vf, struct PIC_BUFFER_CONFIG_s *pic)
{
	...
	if (pbi->chunk && pbi->chunk->signal_type_data_buf) {
...
static void vp9_post_avbcd_task(struct VP9Decoder_s *pbi)
{
	...
	vp9_print(pbi, VP9_DEBUG_BUFMGR,
			"%s: block mode 0x%x (vf %px header_addr 0x%lx y_addr 0x%lx wxh %d x %d bitdepth %d type 0x%lx index %d"
			" offset %d)\n",
			__func__, pbi->mem_map_mode, vf, (ulong)in->img.data, am_buf->planes[0].addr, in->img.rect.width, in->img.rect.height, in->img.bitdep,
...
static int ammvdec_vp9_probe(struct platform_device *pdev)
{
	...
	if (!is_support_triple_write()) {
		if ((pbi->triple_write_mode) || (triple_write_mode)) {
			...
			vp9_print(pbi, 0, "unsupport triple write mode, force disable\n");
		}
	}
	...
	if (!is_support_p010_mode()) {
		if (is_dw_p010(pbi)) {
			...
			vp9_print(pbi, 0, "unsupport dw p010 mode, force disable\n");
		}
		if (is_tw_p010(pbi)) {
			triple_write_mode &= ~(1 <<16);
			pbi->triple_write_mode &= ~(1 <<16);
			vp9_print(pbi, 0, "unsupport tw p010 mode, force disable\n");
		}
	}
	if (no_head & 0x10) {
...
/****************************************/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
static struct param_entry amvdec_vp9_v4l_params[] = {
...
#endif
#ifdef SUPPORT_FB_DECODING
	PARAM_UINT_ARRAY(not_run2_ready),
	PARAM_UINT_ARRAY(run2_count),
	PARAM_UINT(stage_buf_num),
#endif
...
module_param_cb(params, &key_value_param_ops, &amvdec_vp9_v4l_params, 0644);
#endif
...
#ifdef SUPPORT_FB_DECODING
MEDIA_PARAM_ARRAY(not_run2_ready, uint,
	&max_decode_instance_num, 0664);

MEDIA_PARAM_ARRAY(run2_count, uint,
	&max_decode_instance_num, 0664);

MEDIA_PARAM(stage_buf_num, uint, 0664);
