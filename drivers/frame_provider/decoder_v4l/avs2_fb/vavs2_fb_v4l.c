...
static void check_pic_error(struct AVS2Decoder_s *dec, struct avs2_frame_s *pic);
...
static void timeout_process_back(struct AVS2Decoder_s *dec)
{
	...
	avs2_print(dec, 0, "%s decoder timeout HEVC_MPC_E_DBE=0x%x\n",
	...
	unsigned int *mmu_index_adr)
{
	...
	dec->avs2_dec.hc.cur_pic->cur_mmu_4k_number = aml_buf->fbc->frame_size;
...
static int v4l_get_free_fb(struct AVS2Decoder_s *dec)
{
	...
	if (free_pic) {
		...
		if (dec->mmu_enable) {
			dec->afbc_buf_table[dec->aml_buf->fbc->index].POC = dec->avs2_dec.img.tr;
		}
...
/*
+bit[0] 1: reprot HEVC_DECPIC_DATA_ERROR interrupt
+*/
//HEVC_ASSIST_SCRATCH_C
...
int find_near_pic_index(struct AVS2Decoder_s *dec)
{
	...
	for (i = 0; i < BUF_FBC_NUM_MAX; i++) {
		afbc_buf = &dec->afbc_buf_table[i];

		if (((afbc_buf->fb == 0) && (afbc_buf->used == 0)) ||
			(cur_pic->poc == afbc_buf->POC))
			continue;

		if (abs(cur_pic->poc - afbc_buf->POC) < min_diff) {
			min_diff = abs(cur_pic->poc - afbc_buf->POC);
			min_index = i;
		}

		if (abs(cur_pic->poc - afbc_buf->POC) > max_diff) {
			max_diff = abs(cur_pic->poc - afbc_buf->POC);
			max_index = i;
		}
	}
	...
	for (i = 0; i < BUF_FBC_NUM_MAX; i++) {
		...
		avs2_print(dec, PRINT_FLAG_VDEC_DETAIL,
			"%s:i %d afbc_buf->POC %d\n", __func__, i, afbc_buf->POC);
	}
...
static int front_decpic_done_update(struct AVS2Decoder_s *dec, uint8_t reset_flag)
{
	...
	if (!(dec->error_proc_policy & 0x2)
		...
		if (cur_pic->decoded_lcu < dec->avs2_dec.lcu_total) {
			...
			if (index != -1)
				afbc_buf = &dec->afbc_buf_table[index];
			...
			if (afbc_buf != NULL) {
				...
				avs2_print(dec, PRINT_FLAG_VDEC_DETAIL,
					...
					cur_pic->poc, afbc_buf->POC, index, cur_pic->decoded_lcu);
			}
		}
	}
...
static void dump_pic_list(struct AVS2Decoder_s *dec)
{
	...
	for (ii = 0; ii < avs2_dec->ref_maxbuffer; ii++) {
		avs2_print(dec, 0,
		"fref[%d]: pic %px, index %d decode_id %lld mvbuf %d imgcoi_ref %d imgtr_fwRefDistance %d refered %d, pre %d is_out %d, bg %d, vf_ref %d error %d lcu %d poc %d ref_pos(%d,%d,%d,%d,%d,%d,%d) backend_ref %d, back done %d, time %lld addr 0x%x is_display %d\n",
		ii, avs2_dec->fref[ii],
...
static int config_mc_buffer(struct AVS2Decoder_s *dec)
{
	...
	if (avs2_dec->img.type == P_IMG) {
		...
		for (i = 0; i < avs2_dec->img.num_of_references; i++) {
			...
			if ((pic->referred_by_others != 1) && (dec->error_proc_policy & 0x2))
				continue;
			valid_ref_cnt++;
			WRITE_VREG(HEVCD_MPP_ANC_CANVAS_DATA_ADDR,
	...
	while (kfifo_peek(&dec->display_q, &vf) && vf) {
		...
		if (((dec->front_back_mode) && (pic->back_done_mark)) ||
			...
			if (pic->error_drop_flag ||
				...
				avs2_report_err_timestamp_for_decoded_frames(ctx, pic);
				pic->drop_flag = 1;
				vavs2_vf_put(vavs2_vf_get(vdec), vdec);
				avs2_print(dec, AVS2_DBG_BUFMGR, "%s pic poc(%d) has error_mark\n",
					__func__, pic->poc);
				continue;
			} else {
...
static void avs2_recycle_mmu_buf_tail(struct AVS2Decoder_s *dec)
{
	...
	if ((dec->front_back_mode == 1) || (dec->front_back_mode == 3)) {
		pic = dec->avs2_dec.next_be_decode_pic[dec->avs2_dec.fb_rd_pos];
	} else {
		pic = dec->avs2_dec.hc.cur_pic;
	}
...
#ifdef NEW_FB_CODE
		if ((dec->front_back_mode == 1) || (dec->front_back_mode == 3))
			decoder_mmu_box_free_idx_tail(aml_buf->fbc->mmu_1, aml_buf->fbc->index, used_4k_num1);
#endif
		if (pic->need_mmu_copy == 0)
			dec->cur_fb_idx_mmu = INVALID_IDX;
	}
...
{
	if (dec->cur_fb_idx_mmu != INVALID_IDX) {
		struct aml_buf *aml_buf = index_to_aml_buf(dec, dec->cur_fb_idx_mmu);
	...
	struct avs2_frame_s *pic)
{
	...
	if (dec->lcu_percentage_threshold
		...
		pic->error_drop_flag = check_pic_decoded_lcu(dec, pic);
...
static void avs2_buf_ref_process_for_exception(struct AVS2Decoder_s *dec)
{
	...
	if (dec->cur_idx != INVALID_IDX) {
		...
		avs2_dec->frm_pool[cur_idx].imgcoi_ref = -257;
		avs2_dec->frm_pool[cur_idx].imgtr_fwRefDistance = -256;
		avs2_dec->frm_pool[cur_idx].is_output = -1;
...
static irqreturn_t vavs2_isr_thread_fn(int irq, void *data)
{
	...
	if ((dec_status == AVS2_DECODE_BUFEMPTY) || (dec_status == AVS2_DECODE_TIMEOUT)) {
		...
		if (dec->m_ins_flag) {
			...
			update_decoded_pic(dec);
			check_pic_error(dec, pic);
			if (pic && pic->decode_idx == 1) {
				avs2_prepare_display_buf(dec, pic);
			}
#ifdef AVS2_10B_MMU
#ifdef NEW_FB_CODE
			if (dec->front_back_mode == 0)
...
decode_slice:
	...
	if ((dec->front_back_mode == 1) && efficiency_mode) {
		...
		if ((dec->front_back_mode == 1 || dec->front_back_mode == 3) &&
			...
			// 4x4
			WRITE_BACK_8(avs2_dec, HEVC_IQIT_SCALELUT_WR_ADDR, 64); // default seq_wq_matrix_4x4 begin address
			for (i=0; i<16; i++) WRITE_BACK_8(avs2_dec, HEVC_IQIT_SCALELUT_DATA, g_WqMDefault4x4[i]);

			// 8x8
			WRITE_BACK_8(avs2_dec, HEVC_IQIT_SCALELUT_WR_ADDR, 0); // default seq_wq_matrix_8x8 begin address
			for (i=0; i<64; i++) WRITE_BACK_8(avs2_dec, HEVC_IQIT_SCALELUT_DATA, g_WqMDefault8x8[i]);
...
int vavs2_dec_status(struct vdec_s *vdec, struct vdec_info *vstatus)
{
	...
	vdec->vdec_info_statistic.bit_depth = dec->avs2_dec.input.sample_bit_depth;
...
static void avs2_work_implement(struct AVS2Decoder_s *dec)
{
	...
	if (dec->dec_result == DEC_RESULT_AGAIN)
	...
	if ((dec->front_back_mode == 0)
		...
		int index = dec->cur_fb_idx_mmu;
		struct aml_buf *aml_buf = NULL;
		struct mmu_copy *mmu_copy = NULL;

		if (index != INVALID_IDX) {
			aml_buf = index_to_aml_buf(dec, index);
			mmu_copy = &dec->mmu_copy_array[aml_buf->fbc->index];

			if ((mmu_copy->mmu_copy_buf_start) &&
				(dec->avs2_dec.hc.cur_pic->need_mmu_copy != 2) &&
		...
		if (dec->avs2_dec.hc.cur_pic->need_mmu_copy) {
			dec->avs2_dec.hc.cur_pic->used_4k_num = READ_VREG(HEVC_SAO_MMU_STATUS) >> 16;
			avs2_recycle_mmu_buf_tail(dec);
...
void get_mmu_copy_map(struct AVS2Decoder_s *dec, struct avs2_frame_s *pic, u32 page_num)
{
	int i = 0;
	...
	if (buffer_size > mmu_copy->mmu_copy_buf_size) {
		...
		if (mmu_copy->mmu_copy_buf_start && dec->bmmu_box) {
			decoder_bmmu_box_free_idx(dec->bmmu_box,
				MMU_COPY_IDX(aml_buf->fbc->index));
			mmu_copy->mmu_copy_buf_size = 0;
		}
...
static void error_handle_mmu_copy(struct AVS2Decoder_s *dec, struct avs2_frame_s *pic)
{
	...
	page_num = avs2_mmu_page_num(dec, params.err_width,
		params.err_height, bit_depth == AVS2_BITS_10);
...
static int avs2_buffer_recovery(struct AVS2Decoder_s *dec)
{
	...
	if (dec->error_proc_policy & 0x01) {
		if ((kfifo_len(&dec->display_q) <= 0) &&
			...
			lock_buffer(dec, flags);
			for (i = 0; i < avs2_dec->ref_maxbuffer; i++) {
				if (avs2_dec->fref[i]->cma_alloc_addr) {
					decode_count++;
				}
			}
				...
				if (dec->error_proc_policy & 0x2) {
					...
					for (i = 0; i < avs2_dec->ref_maxbuffer; i++) {
						...
						if (pic->cma_alloc_addr) {
							if ((min_idx == 0) || (pic->decode_idx < min_idx)) {
								min_idx = pic->decode_idx;
								index = i;
							}
						}
					}
					...
					if (index != -1) {
						...
						avs2_print(dec, AVS2_DBG_BUFMGR, "dpb buff err, clean dpb poc(%d), index %d\n",
							avs2_dec->fref[index]->poc, index);
						ret = 1;
					}
				}
			}
...
static int get_free_slot(struct AVS2Decoder_s *dec)
{
	struct avs2_decoder *avs2_dec = &dec->avs2_dec;
	int i = 0, free_slot = 0;
...
static bool is_available_buffer(struct AVS2Decoder_s *dec)
{
	...
	free_slot = get_free_slot(dec);

	if (!free_slot) {
		avs2_buffer_recovery(dec);
		avs2_recycle_frame_buffer(dec);
		free_slot = get_free_slot(dec);
	}
...
#ifdef NEW_FB_CODE
	if (dec->front_back_mode == 0)
		WRITE_VREG(HEVC_ASSIST_SCRATCH_C, READ_VREG(HEVC_ASSIST_SCRATCH_C) | 0x1);
#endif
...
static void avs2_dump_state(struct vdec_s *vdec)
{
	...
	avs2_print(dec, 0,
		...
		avs2_print(dec, 0,
			"%s, newq(%d/%d), dispq(%d/%d), vf prepare/get/put (%d/%d/%d), is_available_buffer %d (min %d for run_ready), used_cont %d\n",
			__func__,
...
/****************************************/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
static struct param_entry amvdec_avs2_fb_v4l_params[] = {
	...
	PARAM_UINT(re_search_seq_threshold),
	...
	PARAM_UINT(force_video_signal_type),
	PARAM_UINT(enable_force_video_signal_type),
	...
	PARAM_UINT(back_timer_check_count),
	PARAM_UINT(dump_yuv_frame),
	PARAM_UINT_ARRAY(max_process_time_back),
	PARAM_UINT(efficiency_mode),
#endif
	{ /* sentinel */ }
};
module_param_cb(params, &key_value_param_ops, &amvdec_avs2_fb_v4l_params, 0644);
#endif
...
MEDIA_PARAM(debug_again, uint, 0664);
...
MEDIA_PARAM(re_search_seq_threshold, uint, 0664);
...
MEDIA_PARAM(force_video_signal_type, uint, 0664);
MODULE_PARM_DESC(force_video_signal_type, "\n amvdec_avs2 force_video_signal_type\n");

MEDIA_PARAM(enable_force_video_signal_type, uint, 0664);
...
MEDIA_PARAM(back_timer_check_count, uint, 0664);
MODULE_PARM_DESC(back_timer_check_count,
	"\n avs2 back_timer_check_count\n");

MEDIA_PARAM(dump_yuv_frame, uint, 0664);
