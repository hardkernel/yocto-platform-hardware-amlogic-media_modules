...
static int v4l_parser_work_pic_num(struct hevc_state_s *hevc)
{
	...
	hevc_print(hevc, H265_DEBUG_BUFMGR_MORE, "margin = %d, sps_max_dec_pic_buffering_minus1_0 = %d\n",
	...
	if (hevc->i_only & 0x4) {
		...
		for (i = 0; i < MAX_REF_PIC_NUM; i++) {
			...
			if (pic->slice_type == 2 &&
				...
				hevc_print(hevc, PRINT_FLAG_VDEC_DETAIL, "VH265: output first frame poc %d \n",hevc->first_output_poc);
			}
		}
...
static int config_mc_buffer(struct hevc_state_s *hevc, struct PIC_s *cur_pic)
{
	...
	if (get_dbg_flag(hevc) & H265_DEBUG_BUFMGR)
		hevc_print(hevc, H265_DEBUG_BUFMGR, "config_mc_buffer entered .....\n");
		...
		int decode_pic_begin)
{
	...
	if (is_skip_decoding(hevc, hevc->cur_pic) &&
		(hevc->lcu_percentage_threshold == 0)) {
		if (get_dbg_flag(hevc))
			hevc_print(hevc, H265_DEBUG_DISCARD_NAL,
...
static void v4l_vh265_fill_userdata(struct hevc_state_s *hevc,
	u8 *sei_data_buf, u32 data_len, struct userdata_meta_info_t *meta_info)
{
	struct aml_vcodec_ctx *ctx =
		(struct aml_vcodec_ctx *)(hevc->v4l2_ctx);
	...
	if (get_dbg_flag(hevc) & H265_DEBUG_PRINT_SEI) {
		hevc_print(hevc, 0, "%s userdata: (size %d)\n", __func__, data_len);
	...
	hevc_print(hevc, H265_DEBUG_PRINT_SEI, "%s, ud ready event report\n", __func__);
}
	...
	u32 vpts, u64 vpts_64, int pts_valid, u32 poc)
{
	...
	v4l_vh265_fill_userdata(hevc, sei_data_buf, hevc->sei_itu_data_len, &meta_info);
		...
		if (p+payload_size <= sei_buf+size) {
			...
			case SEI_UserDataUnregister:
				...
				if (!memcmp(p_sei, uuid_and_flag_0, sizeof(uuid_and_flag_0)) ||
					...
					hevc_print(hevc, 0, "SEI payload_type %d\n", payload_type);
...
static struct vframe_s *vh265_vf_get(void *op_arg)
{
	...
	if ((!ctx->avbcd_work_mode && kfifo_get(&hevc->display_q, &vf)) ||
		...
		if (get_dbg_flag(hevc) & H265_DEBUG_PIC_STRUCT) {
			hevc_print(hevc, 0,
				"%s(vf 0x%p type %d flag %x index 0x%x poc %d/%d) pts(%d,%d) dur %d\n",
...
static int notify_v4l_eos(struct vdec_s *vdec)
{
	...
	hevc_print(hw, PRINT_FLAG_VDEC_DETAIL, "[%d] H265 EOS notify.\n", ctx->id);
...
static int vh265_reconfig_mmu_v4l(struct hevc_state_s *hevc, bool enable_mmu)
{
	...
	if (!enable_mmu) {
		...
		hevc_print(hevc, H265_DEBUG_DETAIL, "%s: vh265 mmu clear finish. \n",__func__);
	} else {
		hevc->mmu_enable = 1;
		...
		if (!hevc->frame_mmu_map_addr) {
			pr_err("%s: failed to alloc count_buffer\n", __func__);
			goto frame_mmu_map_addr_failed;
		}
		memset(hevc->frame_mmu_map_addr, 0, get_frame_mmu_map_size());
		...
		hevc_print(hevc, H265_DEBUG_DETAIL, "%s: vh265 mmu enable finish. \n",__func__);
	}

	return 0;

frame_mmu_map_addr_failed:
#ifdef SWAP_HEVC_UCODE
	if (hevc->is_swap) {
		if (hevc->mc_cpu_addr) {
			decoder_dma_free_coherent(hevc->mc_cpu_handle,
			     ...
			     struct aml_vdec_ps_infos *ps)
{
	...
	hevc->sps_num_reorder_pics_0 = rpm_param->p.sps_num_reorder_pics_0;
	...
	if (hevc->interlace_flag) {
		...
		if (ps->bitdepth == 8) {
			dw = DM_YUV_ONLY;
			if (vh265_reconfig_mmu_v4l(hevc, false)) {
				hevc_print(hevc, 0,
	...
	} else if ((hevc->double_write_mode == 0x10) && (ps->bitdepth == 10)) {
		struct aml_vdec_cfg_infos cfg_info = { 0 };
		if (vh265_reconfig_mmu_v4l(hevc, true)) {
			hevc_print(hevc, 0,
				"vh265 mmu clear ERROR! \n");
			return -1;
		}
		if (hevc->double_write_mode_init != 0x10)
			hevc->double_write_mode = hevc->double_write_mode_init;
		else
			hevc->double_write_mode = 3;
		hevc_print(hevc, H265_DEBUG_DETAIL, "h265 10bit can't use dw16 dw %d/%d\n",
						hevc->double_write_mode, hevc->double_write_mode_init);
		vdec_v4l_get_cfg_infos(ctx, &cfg_info);
		cfg_info.double_write_mode = hevc->double_write_mode_init;
		ctx->no_fbc_output = false;
		vdec_v4l_set_cfg_infos(ctx, &cfg_info);
	}
		...
		struct vdec_comp_buf_info *info)
{
	...
	hevc_print(hevc, H265_DEBUG_DETAIL, "hevc get comp info: %d %d %d\n",
...
static int v4l_res_change(struct hevc_state_s *hevc, union param_u *rpm_param)
{
	...
	if (ctx->param_sets_from_ucode) {
		...
		unsigned char interlace_flag = (rpm_param->p.profile_etc >> 2) & 0x1;
		...
		if (hevc->m_ins_flag) {
			...
			if (vdec_frame_based(hw_to_vdec(hevc))) {
				...
				if ((!hevc->discard_dv_data) && (!hevc->dv_duallayer)
					...
					hevc_print(hevc, H265_DEBUG_DV, "dv dual layer\n");
				}
				hevc->empty_flag = 1;
				/*suffix sei or dv meta*/
				set_aux_data(hevc, hevc->cur_pic, 1, 0);
...
muti_output:
			...
			if ((!input_stream_based(vdec) &&
					...
					atomic_read(&hevc->vf_pre_count) < 2)) {
...
static int ammvdec_h265_probe(struct platform_device *pdev)
{
	...
	hevc->double_write_mode_init = hevc->double_write_mode;
...
/****************************************/
/*
 *MEDIA_PARAM(stat, uint, 0664);
 *MODULE_PARM_DESC(stat, "\n amvdec_h265 stat\n");
 */

#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
static struct param_entry amvdec_h265_v4l_params[] = {
	PARAM_UINT(use_cma),
	...
	PARAM_UINT(dirty_time_threshold),
	PARAM_UINT(dirty_count_threshold),
	PARAM_UINT(dirty_buffersize_threshold),
	PARAM_UINT(force_config_fence),
	PARAM_UINT(force_low_latency),
	PARAM_UINT(fence_drop_error_frame),
...
module_param_cb(params, &key_value_param_ops, &amvdec_h265_v4l_params, 0644);
#endif
...
MEDIA_PARAM(dirty_time_threshold, uint, 0664);
MODULE_PARM_DESC(dirty_time_threshold, "\n dirty_time_threshold\n");

MEDIA_PARAM(dirty_count_threshold, uint, 0664);
