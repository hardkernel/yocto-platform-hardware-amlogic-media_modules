...
static void error_handle_mmu_copy(struct AVS2Decoder_s *dec,
...
#define MMU_COPY_IDX(n) (FRAME_BUFFERS + HEADER_FRAME_BUFFERS + 1 + n + 1)
#else
#define MAX_BMMU_BUFFER_NUM (((FRAME_BUFFERS * 3) + HEADER_FRAME_BUFFERS + 1) + 1)
#define VF_BUFFER_IDX(n) (n)
#define HEADER_BUFFER_IDX(n) (FRAME_BUFFERS + n+1)
#define MV_BUFFER_IDX(n) ((FRAME_BUFFERS * 2) + n+1)
#define WORK_SPACE_BUF_ID ((FRAME_BUFFERS * 2) + HEADER_FRAME_BUFFERS+1)
#define MMU_COPY_IDX(n) ((FRAME_BUFFERS * 2) + HEADER_FRAME_BUFFERS + 1 + n + 1)
#endif
	...
	unsigned int *mmu_index_adr)
{
	...
	cur_mmu_4k_number = aml_buf->fbc->frame_size;
	if (get_cpu_major_id() >= AM_MESON_CPU_MAJOR_ID_SM1)
...
int find_near_pic_index(struct AVS2Decoder_s *dec)
{
	...
	if (is_avs2_print_bufmgr_detail()) {
		for (ii = 0; ii < avs2_dec->ref_maxbuffer; ii++) {
			pr_info("fref[%d]: index %d imgcoi_ref %d imgtr_fwRefDistance %d poc %d refered %d, is_out %d, bg %d, vf_ref %d, ref_pos(%d,%d,%d,%d,%d,%d,%d)\n",
...
static void dump_pic_list(struct AVS2Decoder_s *dec)
{
	...
	for (ii = 0; ii < avs2_dec->ref_maxbuffer; ii++) {
		avs2_print(dec, 0,
			"fref[%d]: index %d decode_id %lld poc %d mvbuf %d imgcoi_ref %d imgtr_fwRefDistance %d refered %d, pre %d is_out %d, bg %d, vf_ref %d, error %d, lcu %d, addr:%lx, ref_pos(%d,%d,%d,%d,%d,%d,%d), time %lld\n",
			ii, avs2_dec->fref[ii]->index,
...
static int config_mc_buffer(struct AVS2Decoder_s *dec)
{
	...
	avs2_print(dec, AVS2_DBG_BUFMGR_DETAIL,
		"Entered config_mc_buffer num_of_references %d\n", avs2_dec->img.num_of_references);
...
static void avs2_init_decoder_hw(struct AVS2Decoder_s *dec)
{
	...
	if (get_cpu_major_id() >= AM_MESON_CPU_MAJOR_ID_SM1) {
		...
		data32 = READ_VREG(HEVC_DBLK_CFG0);
		data32 |= (0x1 << 18); // line buffer storage mode, 0:ctu width based, 1:tile width based
		WRITE_VREG(HEVC_DBLK_CFG0, data32);
	}
...
static void avs2_recycle_mmu_buf_tail(struct AVS2Decoder_s *dec)
{
	...
	if (dec->cur_fb_idx_mmu != INVALID_IDX) {
		struct aml_vcodec_ctx *ctx = (struct aml_vcodec_ctx *)(dec->v4l2_ctx);
		struct aml_buf *aml_buf = index_to_afbc_aml_buf(dec, dec->cur_fb_idx_mmu);
		u32 used_4k_num = 0;
	...
	if (pic->decoded_lcu != dec->avs2_dec.lcu_total) {
		...
		if ((!(dec->error_proc_policy & 0x2))
			...
			if (afbc_buf != NULL) {
				...
				avs2_print(dec, AVS2_DBG_BUFMGR_MORE,
					"CUR_POC(%d) decode_idx %d need use mmu copy POC(%d) index %d\n",
					pic->poc, pic->decode_idx, afbc_buf->POC, index);
			}
		}
	} else {
		avs2_print(dec, AVS2_DBG_BUFMGR_MORE,
...
static void avs2_buf_ref_process_for_exception(struct AVS2Decoder_s *dec)
{
	...
	if (dec->cur_idx != INVALID_IDX) {
		...
		avs2_dec->fref[cur_idx]->imgcoi_ref = -257;
		avs2_dec->fref[cur_idx]->imgtr_fwRefDistance = -256;
		avs2_dec->fref[cur_idx]->is_output = -1;
...
static void avs2_work_implement(struct AVS2Decoder_s *dec)
{
	...
	if (dec->avs2_dec.hc.cur_pic
		...
		if (index != INVALID_IDX) {
			aml_buf = index_to_afbc_aml_buf(dec, index);
...
void get_mmu_copy_map(struct AVS2Decoder_s *dec, struct avs2_frame_s *pic, u32 page_num)
{
	...
	for (i = page_num - 1; i < get_frame_mmu_map_size(dec) / 4; i++) {
		map[i] = first_page;
	}
}
	...
	for (i = 0; i < avs2_dec->ref_maxbuffer; ++i) {
		...
		if ((pic->imgcoi_ref < -256) &&
			...
			avs2_print(dec,
				...
				"index %d cma_alloc_addr = 0x%lx, poc %d\n",
...
static void avs2_dump_state(struct vdec_s *vdec)
{
	...
	avs2_print(dec, 0,
		"%s, newq(%d/%d), dispq(%d/%d), vf prepare/get/put (%d/%d/%d), is_available_buffer %d (min %d for run_ready)\n",
		__func__,
...
static int ammvdec_avs2_probe(struct platform_device *pdev)
{
	...
	if (!is_support_triple_write()) {
		if ((dec->triple_write_mode) || (triple_write_mode)) {
			...
			avs2_print(dec, 0, "unsupport triple write mode, force disable\n");
		}
	}
	...
	if (!is_support_p010_mode()) {
		if (is_dw_p010(dec)) {
			...
			avs2_print(dec, 0, "unsupport dw p010 mode, force disable\n");
		}
		if (is_tw_p010(dec)) {
			triple_write_mode &= ~(1 <<16);
			dec->triple_write_mode &= ~(1 <<16);
			avs2_print(dec, 0, "unsupport tw p010 mode, force disable\n");
		}
	}
...
/****************************************/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
static struct param_entry amvdec_avs_v4l_params[] = {
...
module_param_cb(params, &key_value_param_ops, &amvdec_avs_v4l_params, 0644);
#endif
