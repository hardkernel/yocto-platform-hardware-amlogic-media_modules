...
static void vp9_local_uninit(struct VP9Decoder_s *pbi, bool reset_flag)
{
	if (!reset_flag) {
		pbi->rpm_ptr = NULL;
		if (pbi->rpm_addr) {
...
static irqreturn_t vvp9_isr(int irq, void *data)
{
	...
	if (dec_status == VP9_HEAD_PARSER_DONE) {
		vdec_profile(hw_to_vdec(pbi), VDEC_PROFILE_DECODER_HEADER_END, CORE_MASK_HEVC_BACK);
...
static void vp9_work_implement(struct VP9Decoder_s *pbi)
{
	...
	if (pbi->dec_result == DEC_RESULT_AGAIN) {
		vdec_profile(hw_to_vdec(pbi), VDEC_PROFILE_EVENT_AGAIN, CORE_MASK_HEVC_BACK);
...
/****************************************/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
static struct param_entry amvdec_vp9_fb_v4l_params[] = {
	...
	PARAM_UINT_ARRAY(run_count_back),
	PARAM_UINT(test_debug),
	PARAM_UINT(test_dbg),
...
module_param_cb(params, &key_value_param_ops, &amvdec_vp9_fb_v4l_params, 0644);
#endif
...
MEDIA_PARAM_ARRAY(run_count_back, uint,
		&max_decode_instance_num, 0664);

MEDIA_PARAM(test_debug, uint, 0664);
MODULE_PARM_DESC(test_debug, "\n fb_ucode_debug\n");

MEDIA_PARAM(test_dbg, uint, 0664);
MODULE_PARM_DESC(test_dbg, "\n fb_ucode_debug\n");

MEDIA_PARAM(test_schedule, uint, 0664);
